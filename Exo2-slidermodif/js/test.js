(function ($) {
    ...
    function sliderGenerator(sliders) {

        for (const slider of sliders) {
            $(document).on('input', `#slider${slider.sliderId}`, function () {
                let rotation = $(this).val();
                $(slider.sliderValue).html(rotation);
                $(block).css(slider.cssProperty, slider.getCssValue( $(this).val()));
                $(`#btn${slider.sliderId}`).click(function () {
                    $(block).css('transform', 'rotate(0deg)');
                    $(`#slider${slider.sliderId}`).val('0');
                    $(`#slider${slider.sliderId}_value`).html('0');
                });
            });
    ...
    const sliders = [
        {
            ...
            getCssValue: (value) => `rgb(${value},${value},${value})`
        }
    ]
    sliderGenerator(sliders)
